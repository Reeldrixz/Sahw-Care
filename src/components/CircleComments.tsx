...
import { Send, MessageCircle, CornerDownRight, X } from "lucide-react";
...
function firstName(a: CommentAuthor): string {
  return a.circleDisplayName?.trim() || a.name.split(" ")[0];
}
...
export default function CircleComments({ postId, onClose }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    if (res.ok) {
      ...
      const created: Comment = d.comment;
      setComments((prev) => {
        if (created.parentId) {
          // Attach under its top-level parent (server flattens to one level)
          return prev.map((c) =>
  ...
  const renderComment = (c: Comment, isReply: boolean) => {
    const name = firstName(c.author);
    const identity = c.author.circleContext ? `${c.author.circleContext} · ${name}` : name;
    const avatarSize = isReply ? 24 : 28;
    return (
      <div key={c.id} style={{ display: "flex", gap: 10, marginBottom: isReply ? 12 : 14 }}>
        <Avatar src={c.author.avatar} name={c.author.name} size={avatarSize} />
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <button
            ...
            style={{ display: "inline-flex", alignItems: "center", gap: 4, marginTop: 6, background: "none", border: "none", padding: 0, cursor: "pointer", color: "var(--green)", fontSize: 12, fontWeight: 700, fontFamily: "Nunito, sans-serif" }}
          >
            <CornerDownRight size={12} strokeWidth={2.25} /> Reply
          </button>
        </div>
      </div>
    );
      ...
      <div style={{ background: "var(--white)", borderRadius: "24px 24px 0 0", width: "100%", maxWidth: 430, maxHeight: "82vh", display: "flex", flexDirection: "column", animation: "sheetUp 0.3s ease" }}>
        ...
        <div style={{ padding: "14px 20px 12px", borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <div style={{ fontWeight: 800, fontSize: 15 }}>Replies</div>
          </div>
        </div>
            ...
            comments.map((c) => (
              <div key={c.id}>
                ...
                {c.replies && c.replies.length > 0 && (
                  <div style={{ marginLeft: 38, borderLeft: "2px solid var(--border)", paddingLeft: 12, marginBottom: 6 }}>
        ...
        {replyingTo && (
          <div style={{ padding: "8px 16px", background: "var(--bg)", borderTop: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
            <span style={{ fontSize: 12, color: "var(--mid)", fontFamily: "Nunito, sans-serif", display: "inline-flex", alignItems: "center", gap: 5 }}>
              <CornerDownRight size={12} strokeWidth={2.25} color="var(--green)" />
              Replying to <strong style={{ color: "var(--ink)" }}>{replyingTo.name}</strong>
            </span>
            <button
              onClick={() => setReplyingTo(null)}
              style={{ background: "none", border: "none", cursor: "pointer", color: "var(--mid)", display: "flex", padding: 2 }}
        ...
        {/* Compose */}
        <div style={{ padding: "10px 14px 28px", borderTop: replyingTo ? "none" : "1px solid var(--border)", display: "flex", gap: 10, alignItems: "flex-end", flexShrink: 0 }}>
          <textarea
            ref={inputRef}
            value={text}
            onChange={(e) => handleTextChange(e.target.value)}
            placeholder={replyingTo ? `Reply to ${replyingTo.name}…` : "Reply with kindness…"}
            rows={2}
