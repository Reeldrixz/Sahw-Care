...
const EXPORT_MODES: { id: ExportMode; label: string; ratio: string; size: string }[] = [
  { id: "story", label: "Story",  ratio: "9:16", size: "1080×1920" },
  { id: "feed",  label: "Feed",   ratio: "4:5",  size: "1080×1350" },
];
...
export default function ShareImpactModal({ onClose }: Props) {
  ...
  const [exportMode,  setExportMode]  = useState<ExportMode>("story");
  ...
  const handleDownload = async () => {
    const targetRef = exportMode === "story" ? storyRef : cardRef;
    if (!targetRef.current) return;
    setDownloading(true);
    try {
      const { toPng } = await import("html-to-image");
      // Two-pass: first warms the font cache, second captures reliably
      await toPng(targetRef.current, { pixelRatio: 3 });
      const dataUrl = await toPng(targetRef.current, { pixelRatio: 3 });
      const a = document.createElement("a");
      a.href = dataUrl;
      a.download = `kradel-impact-${firstName.toLowerCase()}-${exportMode}.png`;
      a.click();
    } catch {
  ...
  return (
    <>
      {/*
       * Story export canvas — off-screen, always mounted when card is available.
       * 360×640 CSS → 1080×1920 at pixelRatio 3 (9:16).
       * Safe zones: top 85px (≥ 250/3 IG overlay) + card 450px + bottom 105px (≥ 310/3 IG reply bar).
       */}
      {showCard && (
        <div
          ref={storyRef}
          style={{
            position: "fixed", left: -9999, top: 0, zIndex: -1,
            width: 360, height: 640, overflow: "hidden",
            background: "linear-gradient(170deg, #0b3327 0%, #163d2e 50%, #0b3327 100%)",
            display: "flex", flexDirection: "column",
            fontFamily: "Nunito, sans-serif",
          }}
        >
          {/* Top safe zone (85px) — wordmark */}
          <div style={{
            height: 85, flexShrink: 0,
            display: "flex", alignItems: "center", justifyContent: "center",
          }}>
            <div style={{ textAlign: "center" }}>
              <div style={{ fontFamily: "Lora, serif", fontSize: 22, fontWeight: 700, color: "rgba(255,255,255,0.82)", lineHeight: 1 }}>
                Kradəl
              </div>
              <div style={{ fontSize: 9, color: "#7ec8a4", fontWeight: 700, letterSpacing: "1.8px", marginTop: 4, textTransform: "uppercase" }}>
                Impact Story
              </div>
            </div>
          </div>
          ...
          {/* Card zone (450px) */}
          <div style={{ height: 450, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <ImpactCard
          ...
          {/* Bottom safe zone (105px) — tagline */}
          <div style={{
          ...
          <div style={{ padding: "20px 20px 28px" }}>
            {loading ? (
              ...
              <>
                ...
                {/* Camera-roll note — changes by export mode */}
                <div style={{
                  ...
                  <Camera size={13} style={{ flexShrink: 0, marginTop: 1 }} />
                  {exportMode === "story"
                    ? "For Instagram Stories + WhatsApp Status: download and share from your camera roll."
                    : "For Instagram Feed, Facebook, or Twitter: download and post directly."
                  }
                </div>
                  ...
                  {EXPORT_MODES.map(m => (
                    <button
                      ...
                      style={{
                        ...
                        background: exportMode === m.id ? "var(--white)" : "transparent",
                        color:      exportMode === m.id ? "var(--ink)"   : "var(--mid)",
                        fontSize: 12, fontWeight: 800, cursor: "pointer",
                        fontFamily: "Nunito, sans-serif",
                        boxShadow: exportMode === m.id ? "0 1px 4px rgba(0,0,0,0.08)" : "none",
                        transition: "all 0.15s",
                        display: "flex", alignItems: "center", justifyContent: "center", gap: 5,
                      }}
                    >
                      {m.label}
                      <span style={{ fontSize: 10, opacity: 0.55, fontWeight: 700 }}>{m.ratio}</span>
                    </button>
                  ))}
                </div>
                ...
                >
                  {downloading
                    ...
                    : <><Download size={16} /> Download {exportMode === "story" ? "Story (1080×1920)" : "Feed (1080×1350)"}</>
                  }
                </button>
              </>
            )}
