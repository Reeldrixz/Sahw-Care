...
export default function RegisterDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const openItem = async (item: RegisterItemData) => {
    ...
    setMsgText("");
    // Load messages if there's an assignment and user is mom or donor
  ...
  const handleStatusUpdate = async (status: "PURCHASED" | "DELIVERED") => {
    ...
    if (res.ok) {
      setToast(status === "DELIVERED" ? "Marked as delivered! Item fulfilled 🎁" : "Marked as purchased!");
      await fetchRegister();
      setSelectedItem(null);
    } else {
      const d = await res.json();
  ...
  const handleSendMessage = async () => {
    ...
    if (res.ok) {
      ...
      const d = await res.json();
      setToast(d.error ?? "Failed to send");
    }
    setSendingMsg(false);
  };
  ...
  const handleAddItem = async () => {
    ...
    const res = await fetch(`/api/registers/${id}/items`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: newItemName, quantity: newItemQty || "1", note: newItemNote || null }),
    });
    if (res.ok) {
  ...
  return (
    <div style={{ background: "var(--bg)", minHeight: "100vh" }}>
      <div className="discover-desktop">
        {/* Back + header */}
        <div style={{ background: "var(--green)", padding: "16px 16px 24px", color: "white", position: "relative" }}>
          <button
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    boxShadow: "var(--shadow)", cursor: "pointer", display: "flex", alignItems: "center", gap: 12,
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 800, marginBottom: 2, textDecoration: item.status === "FULFILLED" ? "line-through" : "none" }}>
                      {item.name}
                    </div>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 700, padding: "4px 10px", borderRadius: 20, background: cfg.bg, color: cfg.color, flexShrink: 0 }}>
                    {cfg.label}
                  </span>
                </div>
              );
      ...
      {addingItem && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
          <div style={{ background: "var(--white)", borderRadius: "24px 24px 0 0", padding: "20px 16px 40px", width: "100%", maxWidth: 430, animation: "sheetUp 0.3s ease" }}>
            ...
            <div style={{ fontFamily: "Lora, serif", fontSize: 17, fontWeight: 700, marginBottom: 14 }}>Add item to register</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              ...
              <div className="form-group">
                ...
                <input className="form-input" placeholder="Optional" value={newItemNote} onChange={(e) => setNewItemNote(e.target.value)} />
              </div>
            </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }}
          ...
          <div style={{ background: "var(--white)", borderRadius: "24px 24px 0 0", padding: "20px 16px 40px", width: "100%", maxWidth: 430, maxHeight: "90vh", overflowY: "auto", animation: "sheetUp 0.3s ease" }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 13, color: "var(--mid)", fontWeight: 600 }}>Quantity: {selectedItem.quantity}</div>
              ...
              {selectedItem.storeLinks.length > 0 && (
                <div style={{ marginTop: 10 }}>
                  ...
                  {selectedItem.storeLinks.map((link, i) => (
                    <a key={i} href={link} target="_blank" rel="noopener noreferrer" style={{ display: "block", fontSize: 12, color: "var(--green)", fontWeight: 600, marginBottom: 4 }}>
                      🔗 {link}
                    </a>
                  ))}
                </div>
            ...
            {!isMom && user && selectedItem.assignment?.donor.id === user.id && selectedItem.status !== "FULFILLED" && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 8, color: "var(--mid)" }}>Update progress:</div>
            ...
            {/* Chat section — only visible to mom and donor */}
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <button
                    ...
                    style={{ padding: "10px 16px", background: "var(--green)", color: "white", border: "none", borderRadius: 10, fontSize: 14, cursor: "pointer", fontWeight: 700, fontFamily: "Nunito, sans-serif" }}
                  >
                    Send
                  </button>
                </div>
